...
long lttng_counter_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		struct lttng_kernel_counter_map_nr_descriptors __user *user_nr_descriptors =
			(struct lttng_kernel_counter_map_nr_descriptors __user *) arg;
		uint64_t nr_descriptors;

		mutex_lock(&counter->map.lock);
		nr_descriptors = counter->map.nr_descriptors;
		mutex_unlock(&counter->map.lock);
		return put_user(nr_descriptors, &user_nr_descriptors->nr_descriptors);
	}
		...
		struct lttng_kernel_trigger *trigger_param)
{
	...
	if (trigger_param->instrumentation == LTTNG_KERNEL_TRACEPOINT
		 ...
		 * will stay invariant for the rest of the session.
		 */
		trigger = lttng_trigger_create(NULL, trigger_param->token,
		...
		const struct lttng_kernel_counter_conf *error_counter_conf)
{
	...
	ret = lttng_trigger_group_set_error_counter(trigger_group,
			counter_transport_name, counter_len);
	if (ret)
		goto counter_error;

	counter = trigger_group->error_counter;
